...
func TestAllRan(t *testing.T) {
	RegisterTestingT(t)

	t.Run("Returns true with nil pending when no checks are registered", func(t *testing.T) {
		h := setupNewTestHealth()
		ready, pending := h.AllRan()
		Expect(ready).To(BeTrue())
		Expect(pending).To(BeEmpty())
	})

	t.Run("Returns false with all names when registered but not started", func(t *testing.T) {
		h := setupNewTestHealth()
		cfgs := []*Config{
			{Name: "foo", Checker: &fakes.FakeICheckable{}, Interval: testCheckInterval},
			{Name: "bar", Checker: &fakes.FakeICheckable{}, Interval: testCheckInterval},
		...
		Expect(pending).To(ConsistOf("foo", "bar"))
	})

	t.Run("Returns true with empty pending once all checks have ticked", func(t *testing.T) {
		h := setupNewTestHealth()
		cfgs := []*Config{
			{Name: "foo", Checker: &fakes.FakeICheckable{}, Interval: testCheckInterval},
			{Name: "bar", Checker: &fakes.FakeICheckable{}, Interval: testCheckInterval},
